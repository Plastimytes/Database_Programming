-- Active: 1758174916323@@127.0.0.1@3306
...
--Left Outer Join
CREATE VIEW v_User_Progress_Left AS
SELECT
    U.User_ID,
    U.Name,
    U.Role,
...
-- Example
SELECT * FROM v_User_Progress_Left;

--Right Outer Join
CREATE VIEW v_Progress_User_Right AS
SELECT
    U.User_ID,
    U.Name,
    U.Role,
...
-- Example
SELECT * FROM v_Progress_User_Right;

--Full Outer Join
-- The view definition is more complex due to the simulation
CREATE VIEW v_User_Progress_Full AS
(
    SELECT
        U.User_ID,
        U.Name,
...
(
    ...
    RIGHT JOIN
        PROGRESS P ON U.User_ID = P.User_ID
    WHERE
        U.User_ID IS NULL -- Exclude records already covered by the LEFT JOIN
);
